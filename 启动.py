...
def main() -> int:
    ...
    # 命令行/文件管理器给的**媒体文件**：启动后直接开播（"用本程序打开"那条路）。
    # 先 show 出窗口再播 —— 所以这个路径要交给 运行界面，不能在这儿自己开。
    要打开的 = [项 for 项 in argv
              if not 项.startswith("-") and Path(项).is_file()]

    from v8_3.界面.主窗口 import 运行界面
    return int(运行界面(AI运行时=运行时, 主题=主题名,
                      启动日志=启动日志, 打开路径=要打开的) or 0)
