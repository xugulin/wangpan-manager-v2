...
def main() -> int:
    ...
    def 播一遍(地址: str, 请求头: dict | None = None, 标题: str = "") -> dict:
        ...
        引擎.设置暂停(False)
        # 跳转目标要跟素材时长匹配（自带素材只有 2 秒，写死 3.0 会跳到结尾，
        # 看起来像"跳转没生效" —— 真机 CI 上就这么误报过一次）
        目标 = max(0.4, round(引擎.统计.总时长秒 * 0.7, 2)) if 引擎.统计.总时长秒 else 0.8
        结果["跳转目标"] = 目标
        引擎.跳转(目标)
        截止 = time.time() + 6
        while time.time() < 截止 and 引擎.统计.当前时间秒 < 目标 - 0.15:
            time.sleep(0.05)
    ...
    if not 本地["暂停有效"]:
        异常.append("暂停期间还在解码")
    if 本地["跳转后时间"] < 本地["跳转目标"] - 0.4:
        异常.append(f"跳转没生效（目标 {本地['跳转目标']}s，实际 "
                 f"{本地['跳转后时间']}s）")
