...
class 空输出(音频输出设备):
    """没有可用音频设备时的兜底：**按真实时间阻塞**，模拟声卡的节奏。

    为什么必须阻塞（Windows CI 上抓到的真 bug）：
    真实设备（PulseAudio / WASAPI）的 ``写`` 会阻塞到缓冲区有空位，
    所以"已写样本/采样率"天然等于"已经播到第几秒"。而这里的 ``写`` 原本是**立刻返回**，
    于是音频线程几毫秒就把整个文件"写"完了 → 音频时钟瞬间冲到片尾 →
    视频线程永远在等一个已经过去的时刻 → **只出 1 帧**（Windows CI 上就是 25 帧里出 1 帧）。
    所以这里要按样本数 sleep，把节奏还原成真实播放速度。
    """

    名字 = "空输出（没有可用音频设备）"
    #: 允许领先真实时间多少（模拟设备缓冲，别卡太死）
    允许领先秒 = 0.15
    ...
    def 写(self, 数据: bytes) -> bool:
        ...
        应播到 = self._已写样本 / max(1, self.采样率)
        已过 = time.monotonic() - self._开始
        要睡 = 应播到 - 已过 - self.允许领先秒
        if 要睡 > 0:
            time.sleep(min(要睡, 0.5))     # 上限 0.5s：取消/暂停时不能一睡不起
        return True
...
def 打开输出(采样率: int = 48000, 声道数: int = 2, 首选: str = "") -> 音频输出设备:
    ...
    #: 环境变量可以强制后端（排查音频问题 / 在 Linux 上验证 Windows 才会走的空输出路径）
    想要 = (首选 or os.environ.get("V2_音频后端")
          or ("pulse" if os.name != "nt" else "空"))
    if 想要 == "pulse":
        try:
