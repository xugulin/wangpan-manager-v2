...
class _基础解码器:
    ...
    @staticmethod
    def _图像指针数组(帧指针: int, 条数: int = 8):
        """把 AVFrame.data[0..n] / linesize[0..n] 取成 ctypes 数组。

        ⚠️ 默认必须是 **8**（``AV_NUM_DATA_POINTERS``），不能是 4。
        踩过的真机崩溃：音频 5.1（FLTP，6 个平面）时，解码帧有 6 个 data 指针，
        而这里只取了 4 个 → ``swr_convert`` 去读 ``data[4]/data[5]``，
        读到的是**数组外的野指针**，直接越界访问 + 把堆写坏 → 进程随机崩在
        任意 FFmpeg 组件里（真机三次 core 分别在 V2-音频/V2-解封装/V2-缩略图，
        本地 1MB 的 5.1 素材也必崩；立体声因为只有 2 个平面所以一直没事）。
        多取几个指针是安全的：调用方（sws_scale/swr_convert）只会按格式用到需要的几个。
        """
...
class 视频解码器(_基础解码器):
    ...
    def __init__(self, 绑定: B.绑定, 流: 流信息) -> None:
        ...
        self._输出宽 = 0
        self._输出高 = 0
        #: 界面线程写、解码线程读的"请求尺寸"。**只交换这一个元组**：
        #: CPython 里属性赋值是原子的，不会读出"宽是新值、高是旧值"的半成品。
        #: 真正的释放/重建缩放器由解码线程在 _应用输出尺寸请求() 里做（见那里的说明）。
        self._请求尺寸 = (0, 0)
        self.缩放器 = 0
        self._缓冲 = None
        self._目标宽 = 0
        ...
        self.硬解帧数 = 0
        #: 允不允许硬解。默认允许（有设备就挑硬件，没有就软解）；
        #: 界面 / AI 播放顾问可以按需关掉它（"某台机器上硬解花屏/发绿"时的救命开关）。
        #: 环境变量 ``V2_不要硬解`` 是全局兜底，这个字段是**每一路播放**自己的开关。
        self.允许硬解 = True
    ...
    def 配置硬解(self) -> None:
        ...
        * 之后 ``avcodec_receive_frame`` 出来的帧就是**硬件帧**（在显存里），
        ...
        if os.environ.get("V2_不要硬解"):
            ...
            return
        if not getattr(self, "允许硬解", True):
            self.硬解 = "软解（CPU，界面要求关掉硬解）"
            return
        候选 = self._硬解候选()
    ...
    def 设置输出尺寸(self, 宽: int, 高: int) -> None:
        ...
        ⚠️ 这个方法**可能被界面线程调用**（窗口 resize 时），所以它只登记请求、
        绝不碰解码状态 —— 见 :meth:`_应用输出尺寸请求`。
        """
        self._请求尺寸 = (max(0, int(宽 or 0)), max(0, int(高 or 0)))

    def _应用输出尺寸请求(self) -> None:
        """把界面登记的尺寸落到解码状态里。**只能在解码线程里调用。**

        这里必须小心，因为踩过真实的崩溃（用户真机反馈"点两次面板就崩"）：

        原先 ``设置输出尺寸`` 直接在界面线程里 ``sws_freeContext`` 并清零
        ``_目标宽/_目标高``，而解码线程正在 :meth:`取帧` 里用同一个缩放器和这两个值 ——
        - 释放缩放器：解码线程随后把**已释放的指针**交给 ``sws_scale``（use-after-free）；
        - 清零目标尺寸：``取帧`` 算出 ``目标行 = 0``、``输出高 = 0``，却仍按缩放器里的
          旧尺寸往缓冲区写（缓冲区只增不减，但行距/高度对不上，越界写）。

        复现方式（本机 3/3 必崩，改前）：播放中每 2ms 调一次 ``设置输出尺寸`` 随机尺寸，
        进程直接 core dump。所以改成"界面只登记、解码线程在安全点应用"。
        """
        请求 = self._请求尺寸
        if 请求 == (self._输出宽, self._输出高):
            return
        self._输出宽, self._输出高 = 请求
        if self.缩放器:                 # 尺寸变了要重建缩放器（**在本线程里释放**）
    ...
    def _准备缩放(self, 源宽: int, 源高: int, 源格式: int) -> None:
        self._应用输出尺寸请求()      # 安全点：解码线程自己应用界面的尺寸请求
...
class 音频解码器(_基础解码器):
    ...
    def 取块(self) -> Optional[音频块]:
        ...
        # 平面数按**声道数**给够（5.1 是 6 个平面；给 4 会越界读，见上面的说明）
        数据, _行 = self._图像指针数组(帧指针, max(8, int(声道)))
