...
class 播放会话:
    ...
    诊断冷却秒 = 90.0
    #: **两次"自动重开"之间最少隔多久**（秒）。
    #: 真机 22:04:08 同一秒里连发了两条参数更新（`采样并诊断` 与 `_处理调优建议` 各一次），
    #: 两次都走了"换参数重开" —— 同一次播放被重开了两遍，这正是崩溃发生的那一秒。
    #: 去抖以后：第二条只记参数、不再重开（下一条日志就是"参数更新（不重载）"）。
    自动重载最小间隔秒 = 8.0
    #: 参数变了但界面还没到安全点时，排队等多久给一次提示（不刷屏、也不静默）
    待重载提醒秒 = 15.0
    ...
    def __init__(self, *,
        ...
        # ---- 自动重开的安全化（见 应用新参数 / _安排重开 / 推进待办）----
        #: 串行化"换参数重开"的锁（只保护排队状态，起播本身在外面做）
        self._重开锁 = threading.RLock()
        #: 上一次真的重开是什么时候（去抖用）
        self._上次重载时刻 = 0.0
        #: 排队中的参数（等界面到安全点再重开）；None = 没有待办
        self._待重载: Optional[播放设置] = None
        self._待重载时刻 = 0.0
        #: 界面可以挂一个"现在能不能安全重开"的查询（正在拖动/搬窗口时返回 False）。
        #: 默认 None = 任何时候都算安全点（没有界面的场景，比如纯内核/单测）。
        self.安全点查询: Optional[Callable[[], bool]] = None
    ...
    def 应用新参数(self, 新参数: dict, 自动重载: bool = True) -> bool:
        ...
        ⚠️ 自动重开**走同一条安全路径**（见 :meth:`_安排重开`）：去抖 + 等安全点，
        绝不"在用户正拖进度条/正搬独立窗口的时候硬插一次 打开()"。
        """
        老的 = self.设置.to_dict()
        ...
        if not 变了吗 or not 自动重载:
            ...
            return True
        return self._安排重开(设置)

    # ------------------------------------------------------------------ 自动重开

    def _安全点(self) -> bool:
        """现在能不能安全地重开？界面正在拖动进度条/搬独立窗口时不行。

        为什么问界面：自动重开跑在后台诊断线程上，它**看不见**界面此刻在干什么。
        """
        查询 = getattr(self, "安全点查询", None)
        if 查询 is None:
            return True
        try:
            return bool(查询())
    ...
    def _安排重开(self, 设置: 播放设置) -> bool:
        """安排一次"换参数重开"：能安全重开就现在做，否则排队等安全点。

        两道闸门（都是为了真机 22:04:08 那一秒）：

        1. **去抖**：`采样并诊断` 和 `_处理调优建议` 会在同一秒里各提一次参数更新，
           两次都重开 = 同一次播放被重开两遍。间隔不够就只记参数；
        2. **安全点**：正在拖进度条 / 正在把播放页搬进搬出独立窗口时，
           重开要排队，由界面的 16ms 定时器（:meth:`推进待办`）在安全点上做掉 ——
           不硬插、也不丢（参数已经记进 ``self.设置``）。
        """
        现在 = time.monotonic()
        with self._重开锁:
            间隔 = 现在 - self._上次重载时刻
            if 间隔 < self.自动重载最小间隔秒:
                self._待重载 = 设置
                self._待重载时刻 = 现在
                self._记(f"[AI] 刚重开过 {间隔:.1f}s（< {self.自动重载最小间隔秒:.0f}s）："
                        f"这次只记参数、去抖掉重复重开")
                return True
            if not self._安全点():
                self._待重载 = 设置
                self._待重载时刻 = 现在
                self._记("[AI] 现在正在拖动/搬窗口：重开排队等安全点（不硬插）")
                return True
            self._待重载 = None
            self._上次重载时刻 = 现在
        return self._执行重开(设置)

    def 推进待办(self) -> bool:
        """界面每帧（~16ms）调一次：把排队中的"换参数重开"在安全点上做掉。

        返回是否真的做了一次重开。**不持锁做重开**：起播要联网，持锁会把界面卡住。
        """
        现在 = time.monotonic()
        with self._重开锁:
            设置 = self._待重载
            if 设置 is None:
                return False
            if self.引擎.状态 in (内核状态.空闲, 内核状态.结束, 内核状态.出错):
                # 用户已经把播放停了 / 播完了：排队中的重开**作废**。
                # 不这么做的话，界面定时器会在 16ms 后"自作主张"把用户刚停掉的播放重新打开。
                self._待重载 = None
                self._记("[AI] 排队中的参数更新已作废：播放已经停了，绝不替用户重新打开")
                return False
            if not self._安全点():
                if 现在 - self._待重载时刻 >= self.待重载提醒秒:
                    self._待重载时刻 = 现在          # 只提醒一次，不刷屏
                    self._记("[AI] 待重开的参数还在排队：界面还没到安全点"
                            f"（已等 {self.待重载提醒秒:.0f}s，参数不会丢）")
                return False
            if 现在 - self._上次重载时刻 < self.自动重载最小间隔秒:
                return False
            self._待重载 = None
            self._上次重载时刻 = 现在
        return self._执行重开(设置)

    def _执行重开(self, 设置: 播放设置) -> bool:
        """真正重开一路：走**同一条安全路径**（``引擎.打开`` 内部先停干净再建新一代）。

        ⚠️ 不持 ``_重开锁``：起播可能要联网几秒，持锁会把界面的 16ms 定时器卡住。
        并发安全由引擎自己的 ``_开停锁`` 保证（同一次播放只有一个"打开/停止"在跑）。
        """
    ...
    def 关闭(self) -> None:
        """收尾：把这一次的效果学掉，再停内核。"""
        # 排队中的"换参数重开"必须一起作废：否则关页之后它还会去开一路新播放
        with self._重开锁:
            self._待重载 = None
        try:
