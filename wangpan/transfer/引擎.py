...
class 传输引擎:
    ...
    def _下载(self, 适配器对象, 任务对象: 任务, 进度) -> None:
        """下载：HTTP 源优先用**多段并发**（单连接常被 CDN 限速），其它走适配器。

        分段下载自己会探测服务器是否支持 Range；不支持就自动退化为单线程，
        所以"能不能分段"不需要上层判断。
        """
        取消 = lambda: 任务对象.已取消 or 任务对象.已暂停      # noqa: E731
        if getattr(适配器对象, "支持分段下载", False):
            try:
                直链 = 适配器对象.取直链(任务对象.来源路径)
            except Exception:  # noqa: BLE001
                直链 = None
            if 直链 is not None and str(直链.地址).startswith(("http://", "https://")):
                任务对象.总字节 = 任务对象.总字节 or int(直链.大小 or 0)
                分段下载(直链.地址, 任务对象.落点, 任务对象.总字节,
                      段数=任务对象.分段, 请求头=直链.请求头,
                      进度回调=进度, 取消=取消)
                return
        适配器对象.下载(任务对象.来源路径, 任务对象.落点, 进度, 取消=取消)
