...
class Animeko源(弹幕源):
    ...
    def 匹配(self, 素材: 素材信息) -> list[匹配结果]:
        ...
        **有资料库身份时，查词只用库里的中文名/原名**（② 的要求）：文件名可能被改成
        ``146.SDR.8bit…`` 这种，拿它去搜只会搜到不相干的作品；而库里的标题是刮削来的，
        中文名搜不到还能用原名再搜一遍（两个都试，命中率更高）。
        库里没有身份（还没刮削）才退回"用文件名当查词"。
        """
        from ..匹配 import 文件名相似度, 规范化名字
        来自库 = bool(getattr(素材, "来自资料库", False))
        ...
        查询词们: list[str] = []
        if 来自库:
            # 库里给的中文名/原名排在最前面，且**不**把"文件名的其它写法"混进去
            查询词们.extend(素材.搜索词们)
        for 查询 in (干净, 规范化名字(干净) if 干净 else "", 规范化名字(名字), 名字):
            ...
            if 查询 and 查询 not in 查询词们:
                查询词们.append(查询)
        if 来自库:
            # 只用库里给的词（外加"规范化后的同一批词"，上面已经去重保序）；
            # 文件名派生的那些词到此为止 —— 乱改名时它们会搜出别的片子。
            词集 = set(素材.搜索词们) | {规范化名字(x) for x in 素材.搜索词们}
            查询词们 = [x for x in 查询词们 if x in 词集]
        if not 查询词们:
            return []
        结果们: list[匹配结果] = []
        作品们: list[dict] = []
