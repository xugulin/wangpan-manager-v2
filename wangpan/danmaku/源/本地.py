...
__all__ = [
    ...
    "读同名字幕弹幕", "读文本文件", "B站模式号", "B站后缀", "我们的后缀",
    "默认字号", "编码尝试",
]
    ...
    if isinstance(数据, dict):
        ...
        for 键 in ("弹幕", "弹幕们", "comments"):
            if isinstance(数据.get(键), list):
                ...
                break
        if 内层 is None:
            # 顶层是对象又没有弹幕数组：多半是拿错了文件（比如把播放记录传进来了）
            raise 弹幕源错误("弹幕 JSON 的顶层应该是数组（或含 弹幕 数组的对象）")
        数据 = 内层
...
#: 给 读同名字幕弹幕 复用的源（缓存跟着它，别每次调用都重建）
_共用本地源: Optional["本地源"] = None


def 读同名字幕弹幕(视频路径) -> Optional[弹幕池]:
    """给一个视频路径，读它旁边的同名弹幕（找不到就 ``None``）。

    这是播放侧最常用的一个入口：装载在线弹幕时顺手把"用户自己下的同名弹幕"也并进来
    （本地那份往往更合口味）。**不抛异常**：没路径、没文件都只是"没有"。
    """
    global _共用本地源
    if not 视频路径:
        return None
...
class 本地源(弹幕源):
    ...
    def 取弹幕(self, 标识: str) -> 弹幕池:
        ...
        路径 = Path(文本)
        # ⚠️ 只有"看起来就是弹幕文件"才直接读：传进来一个视频路径时它也是 is_file()，
        #    照着读就会拿 mp4 去解 XML/JSON（然后报一堆看不懂的解析错）。
        if 路径.is_file() and 路径.suffix.lower() in (B站后缀 + 我们的后缀):
            文件们 = [路径]
        elif self.自动发现:
            文件们 = 找同名弹幕文件(路径)
        else:
