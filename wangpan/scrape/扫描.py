...
from .模型 import 媒体类型
from .命名解析 import (解析, 解析季目录, 提取发布标签, 猜剧集路径,
                    是同一集的不同版本, 是视频文件)

__all__ = ["发现条目", "扫描结果", "扫描媒体库", "枚举视频单元", "单文件条目",
           "默认跳过目录", "视频扩展名"]
...
def _走目录树(根目录: Path, 结果: 扫描结果, 最小文件字节: int, 上限文件数: int,
          跳过: set):
    """按目录树走一遍，**逐目录**产出 ``(目录, 该目录下的视频们)``。

    为什么要把它抽出来：``扫描媒体库``（按目录归并）与 ``枚举视频单元``（③ 按文件识别）
    的"哪些目录要跳、哪些文件算视频、多小算样片"必须**逐字相同** —— 否则两条通道会对
    同一次扫描给出不同的结论，用户看到的媒体数会莫名变化。
    """
    见过文件 = 0
...
def 扫描媒体库(根目录: Path | str, 最小文件字节: int = 20 * 1024 * 1024,
    ...
    """扫描一个媒体库根目录（**按目录归并**：一部电影/一部剧 = 一个单元）。

    :param 最小文件字节: 小于它的视频当"样片/花絮"跳过（0 = 不跳）。
    """
    结果 = 扫描结果()
    跳过 = {x.lower() for x in 默认跳过目录} | {x.lower() for x in 跳过目录}
    for 目录, 视频们 in _走目录树(Path(根目录), 结果, 最小文件字节, 上限文件数, 跳过):
        结果.条目们.extend(扫描一个目录(目录, 视频们))
    return 结果


def 单文件条目(视频: Path, 目录: Path, 附属: Optional[dict] = None) -> 发现条目:
    """把**一个视频文件**造成一个单元（一个文件 = 一个单元，不跟同目录的其它文件归并）。

    这是 ③"扫描时直接对文件进行识别，不要对文件夹进行识别"的输入形状：用户把若干部
    不同的剧/电影混放在同一个目录里时，"按目录归并"会把它们糊成一部，而一个文件一个
    单元之后，识别（文件名 + 目录名当证据）能各认各的。
    """
    解析结果 = 解析(视频.name, 父目录名=目录.name)
    是剧 = 解析结果.季 is not None or 解析结果.集 is not None
    条目 = 发现条目(
        类型=媒体类型.剧集 if 是剧 else 媒体类型.电影,
        # 季目录归到剧目录（与 `扫描一个目录` 同一约定）：任务路径/重扫都靠它
        路径=目录.parent if 目录.name.lower().startswith("season") else 目录,
        标题=解析结果.标题 or 目录.name, 年份=解析结果.年份,
        季号=解析结果.季, 集号=解析结果.集, 集到=解析结果.集到,
        数据源=解析结果.数据源, 数据源ID=解析结果.数据源ID,
        视频们=[视频], 附属=附属 if 附属 is not None else _收集附属(目录),
        是番外=解析结果.是番外, 特别篇=(解析结果.季 == 0),
        版本=解析结果.版本, 发布标签=提取发布标签(视频.name))
    return 条目


def 枚举视频单元(根目录: Path | str, 最小文件字节: int = 20 * 1024 * 1024,
            上限文件数: int = 200000, 跳过目录: Iterable[str] = ()) -> 扫描结果:
    """扫描一个根目录，但**一个视频 = 一个单元**（不做"按目录归并"）。

    与 :func:`扫描媒体库` 的区别只有一个：不调用 :func:`扫描一个目录` 去把同一个目录里的
    文件归并成"一部电影/一部剧"。跑出来的东西形状一样（``发现条目``），只是粒度是文件。

    为什么需要它（③ 的用户原话：扫描时直接对文件识别，不要对文件夹识别）：混合目录
    里"先按目录归并"会把不是同一部作品的电影整批丢掉（``扫描一个目录`` 一旦认出剧集，
    同目录里没有季集信息的电影就不在结果里了），或者归成一部。
    """
    结果 = 扫描结果()
    跳过 = {x.lower() for x in 默认跳过目录} | {x.lower() for x in 跳过目录}
    for 目录, 视频们 in _走目录树(Path(根目录), 结果, 最小文件字节, 上限文件数, 跳过):
        附属 = _收集附属(目录)
        for 视频 in sorted(视频们):
            结果.条目们.append(单文件条目(视频, 目录, 附属))
    return 结果


def 造远端单元(网盘标识: str, 远端目录: str,
            文件们: list[str]) -> list[发现条目]:
    """把**网盘里的**视频文件造成待刮削单元。

    与本地扫描的关键区别：**一个字节都不碰本地文件系统**。

    为什么需要它：`扫描媒体库()` 走的是 `os.walk` + `目录.iterdir()`，只认本地目录；
    网盘里的文件夹根本没有本地路径可走。而刮削真正依赖的只有**文件名**
    （TMDB 是按名字搜的），所以远端条目照样能刮，只是拿不到 nfo/海报/字幕这些
    附属文件 —— 那些本来就在网盘上，不在本地。

    路径表示法：``标识:远端路径``（例如 ``guangya:/电影/沙丘.2021.mkv``）。
    `资料库.文件.路径` 本来就是 TEXT，这样存既能入库、又能被播放页解析回网盘路径。
    """
    前缀 = f"{网盘标识}:"
    结果: list[发现条目] = []
    ...
    for 远端 in sorted(文件们 or []):
        ...
        结果.append(发现条目(
            类型=类型, 路径=视频.parent, 标题=解析结果.标题 or 视频.stem,
            年份=解析结果.年份, 季号=解析结果.季, 集号=解析结果.集,
            集到=解析结果.集到, 数据源=解析结果.数据源,
            数据源ID=解析结果.数据源ID, 视频们=[视频],
            是番外=解析结果.是番外, 发布标签=提取发布标签(视频.name),
            备注="网盘文件（按文件名刮削）"))
    return 结果


def 合并远端单元(单元们) -> Optional[发现条目]:
    """把"一集一个"的远端单元合成**一部剧一个**。

    用户要求：同一个剧集的不同集要放在媒体库的**一张海报**里、点进去选集；
    以及"确认扫描的信息后剧集的信息没有显示"。

    为什么远端要单独合并：本地扫描靠 `_造单元(路径)` 重新 walk 目录，一部剧自然
    只有一个单元；远端没有目录可 walk（我们手上只有文件列表），扫描时按文件名解析成
    一集一个单元 —— 而 `_本地文件表()/_补季集()/_挂文件()` 期望的是"一个单元里
    带着这一部剧的全部视频"，所以这里按这个形状合起来。
    """
    干净 = [u for u in (单元们 or []) if u is not None and u.主视频 is not None]
    if not 干净:
        return None
    if len(干净) == 1:
        return 干净[0]
    ...
    for u in 干净:
        for v in u.视频们:
            if v not in 视频们:
                视频们.append(v)
    是剧 = any(u.季号 is not None or u.集号 is not None for u in 干净)
    合并 = 发现条目(
        类型=媒体类型.剧集 if 是剧 else 首.类型,
        路径=首.路径, 标题=首.标题, 年份=首.年份,
        季号=首.季号, 集号=首.集号, 集到=首.集到,
        数据源=首.数据源, 数据源ID=首.数据源ID,
        视频们=视频们, 是番外=首.是番外,
        备注=f"网盘 {len(视频们)} 个视频")
    return 合并
