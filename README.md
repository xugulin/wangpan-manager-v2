...
| **TMDB 刮削** `wangpan/scrape/` | 命名解析（官方规范全覆盖 + 研究里 5 个坑的回归）；目录扫描（多版本/多段/番外归并、系统目录跳过）；TMDB 客户端（Bearer/api_key、语言回退、`include_image_language=zh,null`、429 退避、**缓存 180 天硬过期**符合官方条款、aggregate_credits、分级 CN→HK→TW→US 回退、**缓存按端点分区**、凭据杂字符清理）；匹配打分；**待确认队列**（拿不准的**不入库**，候选存进任务表等人点）；**SQLite 11 张表**（含"无 TMDB id 的本地条目按路径各自成行"的去重身份）；图片分级尺寸缓存（LRU+原子写）；NFO 读写；刮削流水线（本地优先、分批、可取消、失败重试） | 单测 **93 条**（含**真 HTTP** 的 10 条：环回假 TMDB 上跑真 urllib/真鉴权/真图片 CDN）；**真机**：扫 3 部同名片 → 2 条进队列 → 采纳「沙丘 2021」→ 落库 + 海报下图 + 挂文件（`工具/真机_待确认队列.py`） |
...
* **刮削**：TMDB 的免费 Key，到 <https://www.themoviedb.org/settings/api> 申请后写进
  ...
  **没配置也能用**：扫描 + 本地 NFO/图片照样入库（验证过）。
  本机网络到不了 `api.themoviedb.org`（域名被解析到无关 IP，图片 CDN 反而是通的）——
  所以"刮削链路"平时靠 `tests/假TMDB服务.py`（环回口上的**真 HTTP** 假 TMDB）验证，
  想看真数据时给个代理即可：`V2_TMDB_API_BASE` / `数据/刮削.json` 的 `apiBase`
  可以把接口指到自建代理；设 `https_proxy` 也行。
...
单元测试：Ran 585 tests … OK (skipped=1)
...
| `工具/检查刮削凭据.py` | 分别报 TMDB 的 **API 域名**与**图片 CDN** 连通性，并列出当前凭据来源（不回显密钥） |
...
tests/                     绑定/解码/硬解/引擎/网络/字幕/体验/网盘传输/独立性/弹幕引擎/弹幕源/
                           ...
                           刮削流水线/待确认队列/海报墙/手动资料/状态栏（585 条）
