...
logger = logging.getLogger("光鸭云盘.界面")
...
class 主界面(QMainWindow):
    ...
    def _构建登录区(self) -> QGroupBox:
        ...
        self.令牌路径标签 = QLabel(f"令牌文件：{全局令牌仓库.文件路径}")
        ...
        self.登录按钮.setToolTip("使用光鸭 App 扫描二维码授权登录")
        self.登录按钮.clicked.connect(self.开始扫码登录)
        按钮行.addWidget(self.登录按钮)

        self.短信登录按钮 = QPushButton("📱 短信登录")
        self.短信登录按钮.setStyleSheet(
            "background: #9C27B0; color: white; padding: 8px 16px; "
            "border-radius: 4px; font-weight: bold;")
        self.短信登录按钮.setToolTip("使用手机短信验证码登录")
        self.短信登录按钮.clicked.connect(self.开始短信登录)
    ...
    def 显示二维码弹窗(self, 链接: str, 用户码: str):
        ...
        logger.info(f"用户码：{用户码}")
        self.状态标签.setText(f"🟡 等待扫码（用户码：{用户码}）")
    ...
    def 开始短信登录(self):
        ...
        对话框 = 短信登录对话框(父窗口=self)
        结果 = 对话框.exec()

        if 结果 == QDialog.Accepted and 对话框.登录结果:
            self._短信登录成功(对话框.登录结果)

    def _短信登录成功(self, 令牌数据: dict):
        logger.info("[界面] 短信登录成功")
        self.状态标签.setText("🟢 已登录（短信登录）")
        self.状态标签.setStyleSheet(
            "font-size: 14px; font-weight: bold; color: green;")
        self.切换登录状态(True)
    ...
    def 重新登录(self):
        ...
        短信按钮 = 选择框.addButton("📱 短信登录", QMessageBox.NoRole)
    ...
    def _标签页切换(self, 索引: int):
        # 云添加页根据 Tab 激活状态控制自动刷新定时器
        self.云添加页.根据Tab激活状态(索引 == 2)
    ...
    def closeEvent(self, 事件):
        logger.info("[界面] 开始关闭窗口，停止定时器和线程...")
        try:
            self.云添加页.定时器.stop()
        except Exception:
            pass
