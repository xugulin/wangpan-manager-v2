# 夸克网盘适配器/界面/页面/文件浏览页.py
"""文件浏览页（含上传/下载/删除/新建文件夹）"""
...
# 阶段一清理：操作类型映射 已随「上传记录」功能删除
from ..字段映射 import 资源类型映射
from ..公共线程 import 批量上传线程
from .基类 import 页面基类

logger = logging.getLogger("夸克网盘.界面.文件浏览")
...
class 文件浏览页(页面基类):
    ...
    def 列目录成功(self, 任务名: str, 数据: dict):
        ...
        for 条目 in 条目列表:
            ...
            # 阶段二 2.5：条目改为夸克原生字段，用 规范条目() 取派生值
            规范 = 文件接口.规范条目(条目)
            是目录 = 规范["是否目录"]
    ...
    def 新建文件夹(self):
        ...
        def 成功(任务名, 结果):
            ...
            # ⚠️ 夸克返回的键是 **`fid`**，不是旧适配器的 `fileId`。
            # 之前只读 `fileId`，于是日志永远是 fileId=None，
            # 而且**创建失败也会被当作成功弹提示**（2026-09-14 GUI 实测暴露）。
            fid = ""
            if isinstance(结果, dict):
                fid = str(结果.get("fid")
                          or (结果.get("data") or {}).get("fid") or "")
            if not fid:
                logger.error(f"[界面] ❌ 新建文件夹 '{名字}' 未返回 fid：{结果}")
                self.状态提示.setText("❌ 新建失败：服务端未返回目录 ID")
                self.状态提示.setStyleSheet(
                    "color: #F44336; padding-left: 10px;")
                QMessageBox.warning(
                    self, "新建失败",
                    f"服务端未返回目录 ID，文件夹可能没有创建成功。\n\n"
                    f"原始响应：{结果}")
                return
            logger.info(f"[界面] ✅ 新建文件夹 '{名字}' fid={fid}")
    ...
    def _下载一个文件(self, 文件ID: str, 本地路径: str):
        ...
        def 任务():
            # 阶段三修复（致命）：必须走 核心.下载.下载服务 ——
            # 它会携带 Cookie、缺 __puus 时自动补全、403 时刷新重试。
            # 之前这里自建 httpx 客户端且不带 Cookie，导致 CDN 直接
            # 返回 403 RequestDeniedByCallback。
            服务 = 下载服务(self.网络)
            目录 = os.path.dirname(本地路径) or "."
            结果 = 服务.下载到文件(
                文件ID, 目录,
                文件名=os.path.basename(本地路径),
                覆盖=True)          # 用户已在保存对话框里确认过路径
            return 结果["path"]
