...
# 阶段三 3.8：下载类型_* 已不再使用（夸克无「下载方式」概念）
from ..字段映射 import (
...
class 分享对话框(QDialog):
    ...
    def _构建界面(self):
        ...
        # ⚠️ 实测（2026-09-14）：单文件分享时夸克**忽略**这里传的 title，
        #    服务端一律用被分享文件的文件名当标题（创建响应、详情回读、
        #    列表三处的 title 都等于文件名）。所以不要给用户「改名会生效」
        #    的错觉，如实说明。
        self.标题输入.setToolTip(
            "提示：夸克服务端对单文件分享会忽略自定义标题，"
            "统一使用被分享文件的文件名。")
        ...
        # 阶段三 3.8：已删除「下载方式 / 流量限制 / 转存次数」三行 ——
        # 夸克分享不支持这些参数（只有 url_type 公开/私密 + passcode，
        # 见 PLAN.md §4.5-D）。
    ...
    def 取配置(self) -> dict | None:
        ...
        # 阶段三 3.8：只保留夸克真正支持的参数
        # （fileIds / title / shareType / validateDuration / code）
        return {
