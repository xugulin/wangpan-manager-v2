...
class 排序与决策测试(unittest.TestCase):
    ...
    def test_自动决策_精确一致的那一个可以直接定(self):
        """真机搜真 TMDB 的场景：满分《沙丘》与 94 分的《沙丘行星》只差 5.8 分。

        这种局面没什么可犹豫的（片名一字不差 + 年份对得上 + 只有它一个），
        所以新规则允许直接采纳 —— 否则待确认队列会被这类条目淹掉。
        """
        候选们 = [电影候选("沙丘", 2021, 标识="1"),
                电影候选("沙丘 2021", 2021, 标识="2")]
        采纳, 需人工, 说明 = 自动决策(候选们, self.目标)
        self.assertIsNotNone(采纳, f"应当直接采纳：{说明}")
        self.assertFalse(需人工)
        self.assertEqual(采纳.标题, "沙丘")

    def test_两个都精确一致还是要人工(self):
        """同名同年两部（不同条目）→ 例外不成立，必须交给人。"""
        候选们 = [电影候选("沙丘", 2021, 标识="1"),
                电影候选("沙丘", 2021, 标识="2")]
        采纳, 需人工, 说明 = 自动决策(候选们, self.目标)
        self.assertIsNone(采纳)
    ...
    def test_精确一致但年份不符要人工(self):
        候选们 = [电影候选("沙丘", 1984, 标识="1"),
                电影候选("沙丘行星", 2021, 标识="2")]
        采纳, 需人工, 说明 = 自动决策(候选们, self.目标)
        self.assertIsNone(采纳)
        self.assertTrue(需人工)
